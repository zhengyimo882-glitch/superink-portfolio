...
function buildInkRiver() {
  ...
  const baseX = pageWidth * (pageWidth < 700 ? .87 : .9);
  const amplitude = clamp(pageWidth * .055, 34, 92);
  const step = clamp(innerHeight * .72, 480, 690);
  ...
  for (let index = 1; index <= segments; index += 1) {
    ...
    const nextX = baseX + Math.sin(index * 1.37) * amplitude;
    const direction = index % 2 ? -1 : 1;
    const controlY1 = previousY + (nextY - previousY) * .34;
    const controlY2 = previousY + (nextY - previousY) * .68;
    const controlX1 = previousX + direction * amplitude * .34;
    const controlX2 = nextX - direction * amplitude * .28;
