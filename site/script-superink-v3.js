...
// Soft section transitions: content enters progressively instead of relying on
// hard background-color boundaries between pages.
...
const inkFx = document.querySelector("#ink-fx");
const reducedMotion = matchMedia("(prefers-reduced-motion: reduce)").matches;

function burstText(element) {
  if (reducedMotion) return;
  const rect = element.getBoundingClientRect();
  const originX = clamp(rect.left + Math.min(rect.width * .18, 110), 24, innerWidth - 24);
  const originY = clamp(rect.top + Math.min(rect.height * .48, 70), 24, innerHeight - 24);
  const colors = ["#00b6a7", "#00d6c3", "#ffe000", "#ff6a21"];
  const count = element.matches("h2") ? 22 : 12;

  for (let i = 0; i < count; i += 1) {
    const particle = document.createElement("i");
    const angle = Math.random() * Math.PI * 2;
    const distance = 24 + Math.random() * (element.matches("h2") ? 95 : 58);
    ...
    particle.style.setProperty("--size", `${2 + Math.random() * 8}px`);
    particle.style.setProperty("--scale", `${.35 + Math.random() * 1.25}`);
    particle.style.setProperty("--rotate", `${Math.random() * 220 - 110}deg`);
