...
// HyperFrames SVG path-draw rule: use measured geometry, never guessed dash lengths.
document.querySelectorAll(".fps-pistol path").forEach((path) => {
...
document.querySelectorAll(
  ...
  surface.appendChild(lock);

  surface.addEventListener("pointerenter", () => surface.classList.add("fps-aiming"));
  surface.addEventListener("pointermove", (event) => {
    const bounds = surface.getBoundingClientRect();
    lock.style.setProperty("--aim-x", `${event.clientX - bounds.left}px`);
    lock.style.setProperty("--aim-y", `${event.clientY - bounds.top}px`);
  });
  surface.addEventListener("pointerleave", () => surface.classList.remove("fps-aiming"));
  surface.addEventListener("pointerdown", () => {
    lock.classList.remove("fps-shot");
    void lock.offsetWidth;
    lock.classList.add("fps-shot");
  });
  lock.addEventListener("animationend", () => lock.classList.remove("fps-shot"));
});
...
document.querySelectorAll("main > section:not(.hero)")
  .forEach((section) => sectionFxObserver.observe(section));
